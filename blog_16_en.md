...
- Conflates "recovery" and "death": it reflects only the death proportion, not the recovery proportion, and therefore struggles to measure the real effect of medical treatment.
...
The state-level data for India come from the [covid19india COVID-19 data](https://github.com/covid19india/data), and the [analyzer tool](https://zyq5945.github.io/StageRecoveryRateAnalyzer/index.html?rf=data/covid19india_states_Recovered.csv&df=data/covid19india_states_Deaths.csv&lang=en&ds=2020-04-01&mode=interval&interval=1&sort=deathDesc&tsort=region&tdir=0&hideIllegal=0&hideInvalid=0) is used to compute the monthly stage recovery rate for each state.
...
The district-level data likewise come from the [covid19india COVID-19 data](https://github.com/covid19india/data), and the [analyzer tool](https://zyq5945.github.io/StageRecoveryRateAnalyzer/index.html?rf=data/covid19india_districts_Recovered.csv&df=data/covid19india_districts_Deaths.csv&lang=en&ds=2020-05-01&mode=interval&interval=1&sort=deathDesc&tsort=region&tdir=0&hideIllegal=0&hideInvalid=0) is used to compute the monthly stage recovery rate for each district.
...
- **Infection–outcome time lag** (see Section 2): the stage recovery rate is computed by outcome time, with a 2–8 week lag relative to infection time. Improvement: realign by infection time, or perform a sensitivity analysis on the lag.
- **Confounding by viral variants and vaccines**: the winter–summer comparison spans the window of Alpha-variant spread and large-scale vaccination. Improvement: compare periods dominated by the same strain, or include variant/vaccine variables in the analysis.
- **Uncontrolled confounding factors**: GDP per capita, population age structure, testing strategy, humidity, and vitamin D levels are all uncontrolled. Improvement: supplement the relevant data and run multiple regression.
- **Changes in the definition of recovery**: the analysis assumes each country's recovery definition remained stable over the period, but in practice it may have been relaxed. Improvement: check the definition-change records of major countries and, if necessary, correct the data.
- **Poor representativeness of the Antarctic case**: research-station personnel are a healthy young population and cannot be generalized to the general public. Improvement: larger-scale data covering the general population are needed.
