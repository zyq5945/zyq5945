...
本文用数据对比分析各国、各地区 COVID-19 的康复情况。先说一个与疫情无关的普遍规律：同一种疾病在冬天的死亡人数明显多于夏天，这是全球普遍现象，有大量流行病学数据支持（具体数据可联网检索）。这一现象背后可能涉及病毒稳定性、室内聚集、维生素 D 缺乏以及人体在寒冷环境下的生理负担等多重因素，未必直接说明温度是独立决定因素。由此可以假设：环境温度偏离舒适区时，人体用于维持体温的能量增加，可能挤占免疫资源，进而影响康复。

这一假设的生理机制是什么？不妨从环境温度偏离程度、身体能量运用和身体反应三个角度对比一下。
...
| 中度偏冷 | 骨骼肌不自主收缩（寒战）产热；能量消耗可翻倍    | 寒战、牙齿打颤、动作僵硬、判断力下降             |
...
还有一个容易混淆的点需要澄清：真正影响身体的是**环境温度**，而非气象观测中的**天气温度**（湿度、风速、辐射也有影响，但相对次要）。天气温度只能算环境温度的粗略代理。南极科考队就是极端例子：所在地天气温度极低，但营地全天候供暖，环境温度得到极大改善，确诊者几乎都是轻症或无症状，没有重症，也没有死亡报告（唯一的南极死亡案例发生在旅游邮轮上）。这侧面印证了环境温度与免疫力的关联，只是科考队员在统计上属于低风险人群，样本太少，不足以当作可靠证据。

因此我的判断是：同一种疫情毒株，寒冷天气下出现重症的概率更高，温暖天气下出现轻症或无症状的概率更高。这个判断是否成立，要用各国真实数据来检验——下面先说明衡量康复情况的计算方式。

需要说明的是，第 1 节的"能量分配"只是一种推测性机制，缺乏直接的临床证据。医学上更被广泛证实的机制包括：冬季维生素 D 缺乏（多项研究证实与新冠重症风险相关）、低湿度削弱呼吸道黏膜防御、冬季室内聚集增加病毒暴露剂量。温度很可能只是这些因素的代理变量，而非独立的直接原因。
...
为了同时观察整体救治水平与特定时段的波动，阶段康复率的起点有两种取法：
...
- 混淆"康复"与"死亡"：它只反映死亡比例，无法反映康复比例，因而难以衡量医疗救治的真实效果。
...
使用阶段康复率还有两个隐含前提需要说明。其一，本文假设各国对"康复"的定义在分析时段内相对稳定——然而实际上不少国家后期把标准放宽为"出院即算康复"，甚至停止追踪康复数，口径变化会人为抬高或压低康复率。其二，阶段康复率统计的是结局发生时间（何时康复/死亡），而感染通常比结局早 2~8 周：冬季时段的病例很多其实感染于秋季，夏季时段则感染于春末初夏。因此它反映的是"该时段内出结局病例"的整体情况，与"该时段感染"之间存在滞后，解读结论时需谨慎。
...
本节使用 [CSSEGISandData的COVID19](https://github.com/CSSEGISandData/COVID-19) 数据，通过 [HTML5阶段康复率分析工具](https://zyq5945.github.io/StageRecoveryRateAnalyzer/index.html?rf=data/CSSEGISandData_COVID-19_recovered_global.csv&df=data/CSSEGISandData_COVID-19_deaths_global.csv&lang=en&ds=2020-04-01&de=2021-08-01&mode=cumSummary&sort=deathDesc&tsort=rate&tdir=-1&hideIllegal=0&hideInvalid=0&filters={"endDeath":{"min":1,"max":null},"stageRec":{"min":1,"max":null},"total":{"min":10000,"max":null}}) （[代码仓库和使用说明](https://github.com/zyq5945/StageRecoveryRateAnalyzer)）分析。为避免数据太少产生噪声，筛选条件如下：阶段康复数不少于 10 000；死亡数不少于 1；置信率 80%~100%；按阶段康复率降序排列。同时补充确诊数、置信度、2020 年人均 GDP 等字段，整理得到下表。
...
表中除基础字段外，还补充了确诊数、置信率、2020 年人均 GDP 三列（置信率定义见下文说明）。

说明：置信率 =（康复数 + 死亡数）÷ 确诊数，越接近 100% 说明病例出结局越完整；低于 80% 说明大量病例尚未报告结局（仍在住院、漏报或口径问题）；置信率超过 100% 说明统计数据错误。

![全程阶段康复率排名前十的地区](images/sec3_top10_zh.jpg)
![全部地区全程阶段康复率（按降序排列，n = 120）](images/sec3_all_zh.jpg)

### 总结

阶段康复率排名前十的地区中，除蒙古外，其余地区的纬度绝对值都在 30° 以内，历史天气普遍偏暖或温热。这十个地区又可分成三个梯队：新加坡是第一梯队且独占一档，卡塔尔、马尔代夫、阿联酋是第二梯队，其余地区是第三梯队。前十地区集中在低纬暖区，这与第 1 节的判断方向一致：温暖环境下康复率更高。

注：蒙古纬度 46.86°，其 2021-07-31 确诊数大于康复数与死亡数之和，数据疑似有误，故不计入纬度统计。
...
注：后来查阅资料发现新加坡死亡统计是 COVID-19 作为根本死因或直接导致死亡的病例，不计算"死亡时携带病毒但死于其他原因"的病例，故阶段康复率无法很好反应其康复情况。但若是有轻重症状数据，其天气特点也可以与其他对照进行比较。
...
本节使用 [CSSEGISandData的COVID19](https://github.com/CSSEGISandData/COVID-19) 数据，选取温度变化较大、数据量也较大的地区，通过 [该分析工具](https://zyq5945.github.io/StageRecoveryRateAnalyzer/index.html?rf=data/CSSEGISandData_COVID-19_recovered_global.csv&df=data/CSSEGISandData_COVID-19_deaths_global.csv&lang=en&ds=2020-04-01&de=2021-08-01&mode=twostage&fs=2020-06-01&fe=2020-09-01&ss=2020-12-01&se=2021-03-01&sort=original&tsort=region&tdir=0&hideIllegal=0&hideInvalid=0&filters={"stageRec":{"min":1,"max":null},"stageDeath":{"min":1,"max":null},"total":{"min":3000,"max":null}}) 分析。为避免数据太少产生噪声，取阶段康复数不少于 3000、康复数和死亡数均不少于 1、2020 年人均 GDP 大于 8000 美元的地区；以 2020-06-01 至 2020-09-01 为第一组对照（北半球夏季/南半球冬季）、2020-12-01 至 2021-03-01 为第二组对照（北半球冬季/南半球夏季），只保留两组时段均有康复率数据的地区，按纬度从大到小排序（|纬度| ≥ 35°），置信率 ≥70%且≤100%（时段拆分后样本量下降，阈值较第 3 节的 80% 放宽至 70%），再补充其他字段，整理得到下表。
...
表中又额外增加了"康复率变化"和"预期"两列。康复率变化是同一地区第二组对照组的康复率减去第一组对照组的康复率得到的差值；预期为 TRUE 表示符合"冬天康复差于夏天"的预期，预期以当地季节为准：北半球 12-2 月为冬季、6-8 月为夏季，南半球相反。

![各地区夏、冬时段阶段康复率对比](images/sec4_compare_zh.jpg)

### 总结

共有 16 个地区作为对照，其中北半球 14 个、南半球 2 个。结果：南半球 2 个均符合"冬天康复差于夏天"；北半球 10 个符合、4 个不符合（可能原因见下文初步假设）。

这个对比存在几个必须承认的混杂因素，结论只能算观察性相关。第一是感染-结局滞后（见第 2 节）：冬夏两组实际对应的是不同季节感染的病例。第二是跨期事件：2020 年底至 2021 年初，Alpha 变种（B.1.1.7）在全球扩散、重症率上升，同期以色列、英国、美国等地开始大规模接种疫苗。冬季时段恰好落在"变种更强、疫苗未普及"的窗口，夏季时段则相反——冬夏差异可能部分来自病毒变种与疫苗的时间线，而非温度本身。第三是统计检验缺失：表内所有差异（如俄罗斯 -0.30%、德国 -1.98%）均未做显著性检验，样本量小的地区差异可能是随机波动，无法判断是信号还是噪声。

北半球 4 个不符合"冬天康复差于夏天"的地区（哈萨克斯坦 +0.87%、保加利亚 +0.72%、意大利 +1.42%、土耳其 +0.89%）也值得单独审视。初步假设包括：2020 年夏季疫情较轻、样本量小导致康复率波动大；康复报告口径在期间可能做过调整；土耳其冬季还大规模使用了特定药物干预。这些假设有待逐一核对各国原始报告确认。
...
印度各州数据来自 [covid19india的COVID19](https://github.com/covid19india/data)，用 [该分析工具](https://zyq5945.github.io/StageRecoveryRateAnalyzer/index.html?rf=data/covid19india_states_Recovered.csv&df=data/covid19india_states_Deaths.csv&lang=en&ds=2020-04-01&mode=interval&interval=1&sort=deathDesc&tsort=region&tdir=0&hideIllegal=0&hideInvalid=0) 按月统计各州的时段阶段康复率。
...
各地区（districts）数据同样来自 [covid19india的COVID19](https://github.com/covid19india/data)，用 [该分析工具](https://zyq5945.github.io/StageRecoveryRateAnalyzer/index.html?rf=data/covid19india_districts_Recovered.csv&df=data/covid19india_districts_Deaths.csv&lang=en&ds=2020-05-01&mode=interval&interval=1&sort=deathDesc&tsort=region&tdir=0&hideIllegal=0&hideInvalid=0) 按月统计各地区（districts）的时段阶段康复率。

数据太多，如需查看各地区明细，可点击本节开头的分析工具链接自行探索。

### 总结

除了病毒和医疗因素，印度夏季的环境温度本身也值得关注。印度每年都有高温致死的记录，夏季频繁出现极端高温天气，部分地区可达第 1 节所述的"中度偏热"甚至"重度偏热"水平；加上人均 GDP 偏低，多数家庭难以通过空调等改善室内环境温度。疫情叠加在这个基础上，反映在数据上，就是不少地区夏天的阶段康复率明显变差。

但必须指出：印度 2021 年 4~7 月这段康复率明显变差的时期，恰好也是 Delta 变种大爆发、医疗系统全面崩溃和氧气短缺的时期，同期全球都观察到印度疫情失控、大量患者因得不到救治而死亡。把康复率下降完全归因于高温，因果推断过于跳跃——医疗挤兑和病毒变种很可能是更直接的原因，高温至多只是叠加因素，这恰恰说明第 1 节所述的"偏离舒适区均不利"只是必要背景，而非充分解释。因此，印度案例只能视为一个"高温叠加疫情"的观察场景，可作为旁证而非确证。
...
数据未经独立核实，仅供参考。目前南极唯一的死亡案例发生在旅游邮轮上，而非科考站。

另外要强调，南极科考队员是经过严格筛选的健康年轻成年人，与普通人群差异很大，即使全员轻症也不能推广到一般人群；这个案例只能算极端环境下的旁证，不具备统计代表性。

---

## 7. 结论

回到开头的问题：温度是否影响 COVID-19 的康复情况？整体来看，[CSSEGISandData的COVID19](https://github.com/CSSEGISandData/COVID-19)（截至 2021-08-01）和 [covid19india的COVID19](https://github.com/covid19india/data)（截至 2021-10-31）的数据都支持这一判断：第 3 节中全程康复率排名前十的地区几乎都集中在低纬度、天气偏暖的区域；第 4 节的冬夏对照中，16 个对照地区里 12 个呈现"冬天康复差于夏天"的规律（北半球 10/14，南半球 2/2），与第 1 节的判断方向一致。

当然也有解释不了的地方：北半球有 4 个地区不符合冬夏规律，可能与夏季样本量小、口径调整或医疗干预有关，尚待核实；南极案例样本太少，只能算旁证。此外，第 5 节的印度提供了一个"高温叠加疫情"的观察场景：极端高温期间康复率同样变差，但该时段与 Delta 变种爆发、医疗挤兑高度重合，不能独立归因于高温，只能作为旁证；其方向与"温度与康复不是线性关系，舒适区附近最好，过冷过热都会拖累康复"的机制一致，但尚不能确证。

所以更准确的结论是：现有数据与"环境温度偏离舒适区不利于康复"的假设方向一致，但鉴于上述混杂因素，目前只能确认**相关性**，无法建立**因果性**。医疗条件、数据口径、人群结构都会起作用，本文用"阶段康复率"这个指标做了一次尝试性对比，指标本身也有局限（依赖数据质量、阶段划分主观）。要得出更可靠的结论，还需要更大样本和更严谨的统计来验证。

与现有研究的关系：新冠的季节性、温度与湿度对传播和重症的影响，以及冬季维生素 D 缺乏与重症风险的关联，已有大量研究（包括发表于《Nature》《Science》等期刊的工作），本文的观察方向与它们大体一致。本文并不宣称发现了新的生物学机制，它的价值在于提出"阶段康复率"这一指标、提供一套可复现的开源分析工具，并为"寒冷季节康复更差"提供一个数据层面的观察视角。

## 8. 局限性与改进方向

**需要更多数据/资源：**

- **感染-结局时间滞后**（详见第 2 节）：阶段康复率按结局时间统计，与感染时间存在 2~8 周滞后。改进：按感染时间重新对齐，或对滞后做敏感性分析。
- **病毒变种与疫苗混杂**：冬夏对比跨越了 Alpha 变种扩散与疫苗大规模接种的窗口。改进：选取同一种毒株主导的时段对比，或在分析中纳入变种/疫苗变量。
- **混杂因素未控制**：人均 GDP、人口年龄结构、检测策略、湿度、维生素 D 水平等均未控制。改进：补充相关数据，做多因素回归。
- **康复口径变化**：假设各国康复定义在时段内稳定，实际可能放宽。改进：核对主要国家的口径变更记录，必要时修正数据。
- **南极案例代表性差**：科考队员为健康年轻人群，不能推广到普通人群。改进：需要更大规模、面向普通人群的数据。

以上意味着"环境温度偏离舒适区不利于康复"目前应视为**相关性观察**，而非因果结论。
